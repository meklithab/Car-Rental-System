...
public:
    ...
    Info peek()
    {
        if (isEmpty())
        {
            cout << "Stack is empty. Cannot peek elements." << endl;
            return Info();
        }
...
void carRentalSystem::returnVehicle()
{
    ...
    string licensePlate;
    cout << "Enter the license plate of the vehicle to return: ";
    cin >> licensePlate;
    ...
    {
        Info rental = RentedVehicles.pop();
        if (rental.licensePlate == licensePlate)
        {
            // Update availability in the linked list
            node *curr = head;
            while (curr != NULL)
            {
                if (curr->data.licensePlate == licensePlate)
                {
                    curr->data.rented = false;
                    curr->data.reserved = false;
                    break;
                }
                curr = curr->next;
            }

            cout << "Vehicle with License Plate " << licensePlate << " has been returned." << endl;
            found = true;
            break;
        }
        else
    ...
    {
        RentedVehicles.push(tempStack.pop());
    }

    if (!found)
    {
        cout << "Vehicle with License Plate " << licensePlate << " not found in the rented vehicles." << endl;
    }
}
bool carRentalSystem::checkAvailability(string licensePlate)
{
    ...
    while (curr != NULL)
    {
        if (curr->data.licensePlate == licensePlate)
        {
            ...
            {
                return false; // Vehicle is not available as it is rented or reserved
            }
            else
            {
...
void carRentalSystem::viewRentedVehicles()
{
    ...
    Stack tempStack = RentedVehicles;

    cout << "Rented Vehicles:" << endl;

    while (!tempStack.isEmpty())
    {
        Info rental = tempStack.pop();
...
int main()
{
    ...
    do
    {
        ...
        cout << "8. Return a Vehicle" << endl;
        cout << "9. Check Vehicle Availability" << endl;
        cout << "10. Exit" << endl;
        cout << "Enter your choice: ";
        cin >> choice;
        ...
        {
            string licensePlate;
            cout << "Enter the licenshe plate of the vehicle: ";
            cin >> licensePlate;
